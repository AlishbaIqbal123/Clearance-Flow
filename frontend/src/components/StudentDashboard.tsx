...
const BentoStatCard = ({ title, value, icon: Icon, color, onClick, description }: { title: string; value: any; icon: any; color: string; onClick?: () => void; description?: string }) => (
  <button 
    className={`
      flex flex-col justify-between p-3 sm:p-5 rounded-xl sm:rounded-2xl bg-card/40 backdrop-blur-md border border-foreground/5 shadow-soft overflow-hidden group relative transition-all duration-700
  ...
  >
    ...
    <div className="flex items-center justify-between relative z-10 w-full mb-2 sm:mb-4">
      <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl ${color} bg-opacity-10 flex items-center justify-center transition-all duration-700 group-hover:scale-110 group-hover:rotate-6 shadow-soft shadow-inner`}>
        <Icon className={`w-4.5 h-4.5 sm:w-5 sm:h-5 ${color.replace('bg-', 'text-')}`} />
      </div>
    ...
    <div className="space-y-1 text-left relative z-10">
      ...
      {description && <p className="text-[7px] font-bold text-muted-foreground/60 uppercase tracking-widest mt-1 sm:mt-1.5">{description}</p>}
    </div>
  </button>
);
...
const PhaseAccordion = ({
  ...
  return (
    <div className={`rounded-xl sm:rounded-2xl border-2 overflow-hidden transition-all duration-500 ${cleared ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-foreground/8 bg-background/30'}`}>
      {/* Accordion Header */}
      <button
        type="button"
        className="w-full flex items-center justify-between px-3.5 py-3 sm:px-6 sm:py-4 gap-2.5 sm:gap-4 hover:bg-foreground/5 transition-colors duration-200"
        onClick={() => setOpen(o => !o)}
      >
        <div className="flex items-center gap-2.5 sm:gap-4">
          <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-lg sm:rounded-xl flex items-center justify-center shrink-0 ${cleared ? 'bg-emerald-500/20' : 'bg-primary/10'}`}>
            {cleared
              ? <CheckCircle2 className="w-3.5 h-3.5 sm:w-4 h-4 text-emerald-500" />
              : <Activity className="w-3.5 h-3.5 sm:w-4 h-4 text-primary animate-pulse" />
            }
          </div>
          <div className="text-left">
            <p className="text-[7px] sm:text-[8px] font-black text-muted-foreground uppercase tracking-[0.4em] leading-none mb-0.5">{phase}</p>
            <h4 className={`text-xs sm:text-sm font-black uppercase tracking-tight leading-none ${cleared ? 'text-emerald-500' : 'text-foreground'}`}>{label}</h4>
          </div>
        </div>
        <div className="flex items-center gap-3 sm:gap-4 shrink-0">
          <span className={`text-[7.5px] sm:text-[9px] font-black uppercase tracking-widest px-2 py-0.5 sm:px-3 sm:py-1 rounded-full ${cleared ? 'bg-emerald-500/15 text-emerald-500' : 'bg-secondary text-muted-foreground'}`}>
            {cleared ? `✓ Completed` : `${clearedCount}/${count} cleared`}
          </span>
          <ChevronRight className={`w-3.5 h-3.5 sm:w-4 h-4 text-muted-foreground transition-transform duration-300 ${open ? 'rotate-90' : ''}`} />
        </div>
      </button>

      {/* Collapsible Content */}
      {open && (
        <div className="px-3.5 pb-4 pt-1.5 sm:px-6 sm:pb-6 sm:pt-2 animate-in fade-in slide-in-from-top-2 duration-300">
          {children}
        </div>
      )}
    </div>
    ...
    <div className="space-y-4 lg:space-y-3 sm:space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-1000">
      {mode === 'fulfillment' ? (
        <div className="space-y-8">
          <div className="relative overflow-hidden rounded-2xl bg-card border border-border/50 p-5 sm:p-8 shadow-strong group">
            <div className="absolute top-0 right-0 w-[45%] h-full bg-primary/20 rounded-full -mr-[20%] -mt-[10%] blur-[100px]" />
            <div className="relative z-10 space-y-2">
               <Badge className="bg-primary/20 text-primary border-none font-black text-[8px] uppercase tracking-[0.4em] px-3 py-1 rounded-full backdrop-blur-md mb-2">Phase 3: Degree Allotment</Badge>
               <h2 className="text-xl sm:text-3xl font-black text-foreground tracking-tighter uppercase leading-none">Degree Fulfillment Portal</h2>
               <p className="text-xs sm:text-sm text-muted-foreground font-medium leading-relaxed italic max-w-2xl">Finalize your degree collection strategy and confirm receipt of your official credentials.</p>
            </div>
          </div>
        ...
        <>
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 lg:gap-6 items-stretch">
            <div className={showSelectionPhase || showStatusPhase ? "lg:col-span-7 flex flex-col" : "lg:col-span-12"}>
              ...
              <div className="relative overflow-hidden rounded-xl sm:rounded-2xl bg-gradient-to-br from-card/95 via-card/85 to-primary/5 border border-border/40 p-3.5 sm:p-5 lg:p-6 shadow-strong group h-full flex flex-col justify-center">
              {/* Dynamic Effects */}
              ...
              <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3 sm:gap-6 lg:gap-10 relative z-10 w-full">
                <div className="space-y-2 sm:space-y-4 max-w-2xl text-left">
                  <div className="space-y-1 sm:space-y-2">
                    <div className="flex items-center gap-2 sm:gap-3">
                      <Badge className="bg-primary/20 text-primary border-none font-extrabold text-[7px] sm:text-[8px] uppercase tracking-[0.2em] px-2 py-0.5 sm:px-3 sm:py-0.5 rounded-full backdrop-blur-md shadow-sm">
                         Student Dashboard
                      </Badge>
                    ...
                    <h2 className="text-[13px] sm:text-lg lg:text-xl font-extrabold text-foreground tracking-tight leading-tight sm:leading-none uppercase">
                      {activeRequest?.status === 'fully_cleared' ? (
                  ...
                  <p className="text-[9.5px] sm:text-xs text-muted-foreground font-medium leading-relaxed max-w-xl opacity-80 italic">
                    {activeRequest?.status === 'fully_cleared' 
                  ...
                  <div className="flex flex-wrap gap-1.5 pt-0.5">
                    {[
                      ...
                      <div key={i} className="flex items-center gap-1 sm:gap-2 bg-secondary/50 border border-border/30 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full hover:bg-secondary/70 transition-colors">
                        <tag.icon className="w-2 h-2 sm:w-3 sm:h-3 text-primary" />
                        <div className="flex flex-col">
                           <span className="text-[5px] sm:text-[6px] font-black text-muted-foreground/50 uppercase tracking-widest leading-none mb-0.5">{tag.label}</span>
                           <span className="text-[7.5px] sm:text-[9px] font-black text-foreground uppercase tracking-tight">{tag.value}</span>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {canSubmitNewRequest && (
                  <Button 
                    ...
                    disabled={submitting}
                    className="w-full sm:w-auto bg-primary text-white hover:bg-primary/90 h-8 sm:h-9 px-3.5 sm:px-4 rounded-lg font-black text-[7.5px] sm:text-[9px] uppercase tracking-[0.2em] shadow-strong shadow-primary/20 group shrink-0 active:scale-95 transition-all relative overflow-hidden"
                  >
                    ...
                    <div className="flex items-center gap-2 relative z-10">
                      {submitting ? (
                        ...
                        <div className="w-5.5 h-5.5 bg-white/20 rounded-md flex items-center justify-center group-hover:scale-110 group-hover:rotate-6 transition-transform">
              ...
              {showStatusPhase && (
                <div className="animate-in zoom-in-95 slide-in-from-top-12 duration-1000 ease-out h-auto lg:h-full flex">
                  <Card className={`border-none shadow-strong rounded-xl sm:rounded-2xl lg:rounded-3xl ${activeRequest.status === 'fully_cleared' ? 'bg-gradient-to-br from-card/90 via-card/75 to-primary/5 border border-border/40 text-foreground' : 'bg-gradient-to-br from-emerald-950 via-emerald-900 to-emerald-950 border border-emerald-500/20 text-white'} overflow-hidden relative group transition-colors duration-700 h-auto lg:h-full w-full flex flex-col justify-center`}>
                    ...
                    <div className="flex flex-col items-center lg:items-start text-center lg:text-left gap-2.5 p-3 sm:p-5 lg:p-6 relative z-10 h-auto lg:h-full justify-center w-full">
                      <div className={`w-7 h-7 sm:w-9 sm:h-9 ${activeRequest.status === 'fully_cleared' ? 'bg-primary/10 border border-primary/20' : 'bg-emerald-500/20 border border-emerald-500/30'} rounded-lg sm:rounded-xl flex items-center justify-center backdrop-blur-xl shrink-0`}>
                        {activeRequest.status === 'fully_cleared' ? <ShieldCheck className="w-3 h-3 sm:w-4 sm:h-4 text-primary" /> : <Truck className="w-3 h-3 sm:w-4 sm:h-4 text-emerald-400" />}
                      </div>
                      
                      <div className="space-y-1">
                        <div className="space-y-0.5">
                          <Badge className={`${activeRequest.status === 'fully_cleared' ? 'bg-primary text-white' : 'bg-emerald-500 text-white'} border-none font-extrabold text-[6.5px] sm:text-[8px] uppercase tracking-[0.2em] px-2 py-0.5 sm:px-3 sm:py-1 rounded-full shadow-soft mb-0.5`}>
                            {activeRequest.status === 'fully_cleared' ? 'CLEARANCE COMPLETE' : 'Phase 3: Degree Delivery'}
                          </Badge>
                          <h3 className="text-xs sm:text-sm lg:text-base font-extrabold tracking-tight uppercase leading-tight">
                            {activeRequest.status === 'fully_cleared' 
                        ...
                        <p className={`text-[8.5px] sm:text-xs font-medium ${activeRequest.status === 'fully_cleared' ? 'text-muted-foreground/90' : 'text-emerald-100/80'} leading-relaxed max-w-xl`}>
                          {activeRequest.status === 'fully_cleared'
                      ...
                      <div className="flex flex-col sm:flex-row lg:flex-col gap-1.5 w-full mt-1">
                        {activeRequest.degree_fulfillment.notification_sent && !activeRequest.degree_fulfillment.received_by_student && (
                          <Button 
                            className="h-8 sm:h-10 px-3 sm:px-5 rounded-full bg-primary text-white hover:bg-primary/90 font-black text-[7.5px] sm:text-[9px] uppercase tracking-[0.15em] sm:tracking-[0.2em] transition-all active:scale-95 flex items-center gap-1.5 sm:gap-2.5 w-full sm:w-auto lg:w-full sm:min-w-[220px] lg:min-w-0 shadow-soft animate-pulse"
                          ...
                          >
                            ...
                            <div className="text-left">
                              ...
                              <span className="block text-[5px] sm:text-[6px] text-white/60 mt-0.5 font-bold">Finalize Clearance Now</span>
                            </div>
                          </Button>
                        )}
                        
                        <Button 
                          className={`h-8 sm:h-10 px-3 sm:px-5 rounded-full ${activeRequest.status === 'fully_cleared' ? 'bg-primary/10 text-primary hover:bg-primary/15' : 'bg-white/90 text-emerald-950 hover:bg-white'} font-black text-[7.5px] sm:text-[9px] uppercase tracking-[0.15em] sm:tracking-[0.2em] transition-all active:scale-95 flex items-center gap-1.5 sm:gap-2.5 w-full sm:w-auto lg:w-full sm:min-w-[220px] lg:min-w-0 shadow-soft`}
                          onClick={() => window.location.href = 'mailto:registrar@university.edu?subject=Clearance Fulfillment Inquiry'}
                        >
                          <div className={`w-5 h-5 ${activeRequest.status === 'fully_cleared' ? 'bg-primary/20' : 'bg-emerald-100'} rounded-full flex items-center justify-center shrink-0`}>
                            <MessageSquare className="w-3 h-3 text-primary" />
                          </div>
                          <div className="text-left">
                            <span className="block font-black">Institutional Support</span>
                            <span className="block text-[5px] sm:text-[6px] opacity-60 mt-0.5 font-bold">Support ID: {activeRequest.id.slice(0, 8)}</span>
                          </div>
                        </Button>
                      </div>
                    </div>
