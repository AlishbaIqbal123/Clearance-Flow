...
export const DepartmentChats: React.FC<DepartmentChatsProps> = ({ user }) => {
  ...
  const threads = requests.map(req => {
    const comments = req.comments || [];
    const deptId = user?.department_id;
    // Filter comments strictly addressed to this department, but allow legacy generic messages to maintain history
    ...
    const lastMessage = deptComments[deptComments.length - 1];
