...
export const Analytics = ({ user }: { user: any }) => {
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.setFillColor(0, 102, 255); // Primary color
      ...
      doc.text('CUI VEHARI | CLEARANCE SYSTEM', 45, 22);
      
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      doc.text(`INSTITUTIONAL PERFORMANCE AUDIT - ${new Date().toLocaleDateString()}`, 45, 30);
      
      // Report Info
      doc.setTextColor(0, 0, 0);
      doc.setFontSize(14);
      doc.text('ANALYTICS OVERVIEW', 15, 55);
      
      doc.setFontSize(10);
      doc.text(`Generated by: ${user?.firstName} ${user?.lastName} (${user?.role?.toUpperCase()})`, 15, 62);
      if (user?.department?.name) {
        doc.text(`Department: ${user.department.name}`, 15, 68);
      }
      ...
      const summaryData = [
        ...
        ['Cleared Requests', data?.statusBreakdown?.cleared || 0],
        ['Pending Requests', data?.statusBreakdown?.pending || 0],
        ['In Review', data?.statusBreakdown?.in_review || 0],
      ...
      (doc as any).autoTable({
        ...
        body: summaryData,
        theme: 'striped',
        headStyles: { fillStyle: [0, 102, 255] }
      });
      ...
      if (data?.departmentPerformance?.length > 0) {
        ...
        (doc as any).autoTable({
          ...
          head: [['Department', 'Rate', 'Total', 'Cleared', 'Pending']],
          body: deptData,
          theme: 'grid',
          headStyles: { fillStyle: [0, 102, 255] }
        });
      ...
      if (data?.batchDistribution?.length > 0) {
        const batchData = data.batchDistribution.map((b: any) => [b.batch, b.count]);
        
        doc.addPage();
        doc.setTextColor(0, 0, 0);
        doc.setFontSize(14);
        doc.text('STUDENT BATCH DISTRIBUTION (PENDING)', 15, 20);

        (doc as any).autoTable({
          startY: 25,
          head: [['Batch', 'Pending Students']],
          body: batchData,
          theme: 'striped',
          headStyles: { fillStyle: [0, 102, 255] }
        });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4 space-y-8">
           ...
           <Card className="border-none shadow-soft rounded-3xl bg-primary text-white group overflow-hidden relative cursor-pointer active:scale-95 transition-all duration-700">
              ...
              <CardContent className="p-6 sm:p-8 relative z-10 flex flex-col items-center text-center space-y-6 sm:space-y-8">
                 ...
                 <Button 
                    onClick={exportToPDF}
                    disabled={isExporting}
                    className="w-full h-12 sm:h-14 bg-white text-primary rounded-xl sm:rounded-2xl font-black text-[9px] sm:text-[10px] uppercase tracking-[0.5em] shadow-strong hover:bg-indigo-50 transition-all active:scale-90"
                  >
                    {isExporting ? 'Syncing...' : 'Audit Report'}
           ...
           {batchDistribution.length > 0 && (
             <Card className="border-none shadow-soft rounded-3xl bg-card/60 backdrop-blur-3xl border border-foreground/5 group overflow-hidden mt-12">
               <CardHeader className="p-5 sm:p-6 pb-4 border-b border-foreground/5">
                 <div className="flex items-center gap-4">
                   <div className="w-9 h-9 sm:w-10 sm:h-10 bg-indigo-500/10 rounded-xl flex items-center justify-center text-indigo-500 group-hover:rotate-6 transition-transform duration-700 shadow-soft">
                     <GraduationCap className="w-4.5 h-4.5 sm:w-5 h-5" />
                   </div>
                   <div className="space-y-0.5">
                     <CardTitle className="text-base sm:text-lg font-black tracking-tighter uppercase leading-none">Batch Distribution</CardTitle>
                     <CardDescription className="font-black text-[8px] uppercase tracking-[0.3em] opacity-40 mt-0.5 italic">Pending requests by academic year.</CardDescription>
                   </div>
                 </div>
               </CardHeader>
               <CardContent className="p-5 sm:p-6 grid grid-cols-2 md:grid-cols-4 gap-6">
                 {batchDistribution.map((b: any, i: number) => (
                   <div key={i} className="bg-secondary/20 p-5 rounded-2xl border border-foreground/5 space-y-1 group/batch hover:bg-primary/5 transition-colors">
                     <p className="text-[10px] font-black text-muted-foreground uppercase tracking-widest opacity-40 leading-none group-hover/batch:text-primary transition-colors">Batch {b.batch}</p>
                     <p className="text-2xl font-black text-foreground tracking-tighter leading-none">{b.count}</p>
                     <p className="text-[7px] font-black text-muted-foreground uppercase tracking-widest">Students Pending</p>
                   </div>
                 ))}
               </CardContent>
             </Card>
           ...
           {data?.recentPending?.length > 0 && (
             <Card className="border-none shadow-soft rounded-3xl bg-card/60 backdrop-blur-3xl border border-foreground/5 group overflow-hidden mt-12">
               <CardHeader className="p-5 sm:p-6 pb-4 border-b border-foreground/5">
                 <div className="flex items-center gap-4">
                   <div className="w-9 h-9 sm:w-10 sm:h-10 bg-amber-500/10 rounded-xl flex items-center justify-center text-amber-500 group-hover:rotate-6 transition-transform duration-700 shadow-soft">
                 ...
                 <div className="divide-y divide-foreground/5">
                   {data.recentPending.map((req: any, i: number) => (
                     <div key={i} className="p-5 flex items-center justify-between hover:bg-primary/5 transition-colors group/req">
                       <div className="flex items-center gap-4">
                         <div className="w-10 h-10 rounded-full bg-secondary flex items-center justify-center font-black text-xs text-primary border border-foreground/5 group-hover/req:scale-110 transition-transform">
                       ...
                       <Badge variant="outline" className="bg-amber-500/10 text-amber-600 border-none rounded-lg px-3 py-1 text-[8px] font-black uppercase tracking-widest">
                         Pending
                       </Badge>
                     </div>
                   ))}
